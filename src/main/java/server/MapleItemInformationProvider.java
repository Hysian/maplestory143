...
//        System.out.println("共加载了 " + WeaponIDs.size() + " 个武器");
    ...
    public void loadSetItemData() {
        if (!RedisUtil.exists(KEYNAMES.SETITEM_DATA.getKeyName())) {
            ...
            for (MapleData dat : setsData) {
                ...
                try {
                    RedisUtil.hset(KEYNAMES.SETITEM_DATA.getKeyName(), dat.getName(), mapper.writeValueAsString(SetItem));
    ...
    public void loadPotentialData() {
        if (!RedisUtil.exists(KEYNAMES.POTENTIAL_DATA.getKeyName())) {
            ...
            for (MapleData dat : potsData) {
                ...
                try {
                    RedisUtil.hset(KEYNAMES.POTENTIAL_DATA.getKeyName(), String.valueOf(Integer.valueOf(dat.getName())), mapper.writeValueAsString(items));
                } catch (JsonProcessingException e) {
                    e.printStackTrace();
                }
            }
    ...
    public void loadSocketData() {
        if (!RedisUtil.exists(KEYNAMES.SOCKET_DATA.getKeyName())) {
            ...
            for (MapleData dat : nebuliteData) {
                ...
                try {
                    RedisUtil.hset(KEYNAMES.SOCKET_DATA.getKeyName() + ":" + ItemConstants.getNebuliteGrade(item.opID), String.valueOf(item.opID), mapper.writeValueAsString(item));
                } catch (JsonProcessingException e) {
                    e.printStackTrace();
                }
            }
            RedisUtil.set(KEYNAMES.SOCKET_DATA.getKeyName(), "true");
        }
    ...
    public void runItems() {
         ...
         * 读取所有物品名称、描述
         */
        if (!RedisUtil.exists(KEYNAMES.ITEM_DATA.getKeyName())) {
            ...
            for (MapleData namedata : mapleDatas) {
                ...
                for (Pair<String, String> type : types) {
                    RedisUtil.hset(type.getLeft(), itemid, MapleDataTool.getString(namedata.getChildByPath(type.getRight()), ""));
                }
            }
            ...
            RedisUtil.hmset(KEYNAMES.ITEM_DATA.getKeyName(), itemDataMap);
            if (!specialItemNameMap.isEmpty()) {
                RedisUtil.hmset(KEYNAMES.ITEM_NAME.getKeyName(), specialItemNameMap);
            }
            if (!specialItemDescMap.isEmpty()) {
                RedisUtil.hmset(KEYNAMES.ITEM_DESC.getKeyName(), specialItemDescMap);
            }
            log.info("共加载 " + RedisUtil.hlen(KEYNAMES.ITEM_DATA.getKeyName()) + " 个道具信息.");
            ...
            for (MapleDataDirectoryEntry topDir : root.getSubdirectories()) {
                ...
                for (MapleDataFileEntry iFile : topDir.getFiles()) {
                    ...
                    RedisUtil.lpush(KEYNAMES.HAIR_FACE_ID.getKeyName(), idstr);
                }
            }
        }
    ...
    public List<StructItemOption> getPotentialInfo(int potId) {
        String json = RedisUtil.hget(KEYNAMES.POTENTIAL_DATA.getKeyName(), String.valueOf(potId));
    ...
    public Map<Integer, List<StructItemOption>> getPotentialInfos(int potId) {
        ...
        Map<String, String> data = RedisUtil.hgetAll(KEYNAMES.POTENTIAL_DATA.getKeyName());
    ...
    public StructItemOption getSocketInfo(int socketId) {
        ...
        String json = RedisUtil.hget(KEYNAMES.SOCKET_DATA.getKeyName() + ":" + grade, String.valueOf(socketId));
    ...
    public Map<Integer, StructItemOption> getAllSocketInfo(int grade) {
        ...
        for (Entry<String, String> entry : RedisUtil.hgetAll(KEYNAMES.SOCKET_DATA.getKeyName() + ":" + grade).entrySet()) {
            try {
    ...
    public Map<String, String> getAllItemNames() {
        return RedisUtil.hgetAll(KEYNAMES.ITEM_NAME.getKeyName());
    }

    public long getAllItemSize() {
        return RedisUtil.hlen(KEYNAMES.ITEM_DATA.getKeyName());
    }
...
//    }
    ...
    private <T> T getItemProperty(int itemId, String path, T defaultValue) {
        String json = RedisUtil.hget(KEYNAMES.ITEM_DATA.getKeyName(), String.valueOf(itemId));
        if (json == null) {
            return defaultValue;
        }
        Map<?, ?> data = null;
...
//            return !ServerConfig.WORLD_EQUIPCHECKFAME && (fameReq == null || fame >= fameReq);
    ...
    public StructSetItem getSetItem(int setItemId) {
        ...
        try {
            ret = mapper.readValue(RedisUtil.hget("SetItemInfo", String.valueOf(setItemId)), StructSetItem.class);
        } catch (IOException e) {
            e.printStackTrace();
        }
        return ret;
    ...
    */
    ...
    public String getName(int itemId) {
        return RedisUtil.hget(KEYNAMES.ITEM_NAME.getKeyName(), String.valueOf(itemId));
    }
    ...
    public String getDesc(int itemId) {
        return RedisUtil.hget(KEYNAMES.ITEM_DESC.getKeyName(), String.valueOf(itemId));
    }

    public String getMsg(int itemId) {
        return RedisUtil.hget(KEYNAMES.ITEM_MSG.getKeyName(), String.valueOf(itemId));
    }
    ...
    public boolean itemExists(int itemId) {
        ...
        return RedisUtil.hexists(KEYNAMES.ITEM_DATA.getKeyName(), String.valueOf(itemId));
    }
...
//        }
    ...
    public boolean loadHairFace(int id) {
        return RedisUtil.lrange(KEYNAMES.HAIR_FACE_ID.getKeyName(), 0, -1).contains(String.valueOf(id));
    }
