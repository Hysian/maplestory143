...
public class MapleMapFactory {
    ...
    public static void loadAllMapName() {
        try {
            if (!RedisUtil.exists(RedisUtil.KEYNAMES.MAP_NAME.getKeyName())) {
                    ...
                    for (MapleData data : mapleData) {
                        for (MapleData subdata : data) {
                            if (subdata.getName().equalsIgnoreCase("mapName")) {
                                RedisUtil.hset(RedisUtil.KEYNAMES.MAP_NAME.getKeyName(), data.getName(), subdata.getData().toString());
                            }
                        }
                    }
                }
    ...
    public static void loadAllLinkNpc() {
        if (!RedisUtil.exists(RedisUtil.KEYNAMES.MAP_LINKNPC.getKeyName())) {
                            ...
                            for (MapleDataFileEntry fileEntry : directoryEntry1.getFiles()) {
                                for (MapleData life : source.getData(directoryEntry.getName() + "/" + directoryEntry1.getName() + "/" + fileEntry.getName())) {
                                    if (life.getName().equals("life")) {
                                        ...
                                        if (!npcids.isEmpty()) {
                                            try {
                                                RedisUtil.hset(RedisUtil.KEYNAMES.MAP_LINKNPC.getKeyName(), Integer.valueOf(fileEntry.getName().substring(0, 9)).toString(), JsonUtil.getMapperInstance().writeValueAsString(npcids));
