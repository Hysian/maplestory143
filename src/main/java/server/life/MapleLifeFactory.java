...
public class MapleLifeFactory {
    ...
    static {
        // 将怪物名字缓存
        if (!RedisUtil.exists(KEYNAMES.MOB_NAME.getKeyName())) {
            for (MapleData mapleData : mobStringData) {
                RedisUtil.hset(KEYNAMES.MOB_NAME.getKeyName(), mapleData.getName(), String.valueOf(mapleData.getChildByPath("name").getData()));
            }
        }

        if (!RedisUtil.exists(KEYNAMES.MOB_ID.getKeyName())) {
            ...
            for (MapleDataProvider root : datas) {
                for (MapleDataFileEntry mapleDataFileEntry : root.getRoot().getFiles()) {
                    ...
                    RedisUtil.hset(KEYNAMES.MOB_ID.getKeyName(), String.valueOf(mobid), String.valueOf(isboss));
                }
            }
        }
    }
    ...
    public static String getMonsterName(int mobid) {
        return RedisUtil.hget(KEYNAMES.MOB_NAME.getKeyName(), String.valueOf(mobid));
    }

    public static boolean isBoss(int mobid) {
        return Boolean.valueOf(RedisUtil.hget(KEYNAMES.MOB_ID.getKeyName(), String.valueOf(mobid)));
    }

    public static boolean checkMonsterIsExist(int mobid) {
        return RedisUtil.hexists(KEYNAMES.MOB_ID.getKeyName(), String.valueOf(mobid));
    }
    ...
    public static void loadQuestCounts() {
        if (!RedisUtil.exists(KEYNAMES.QUEST_COUNT.getKeyName())) {
                    ...
                    if (mapz.getName().equals("QuestCountGroup")) {
                        for (MapleDataFileEntry entry : mapz.getFiles()) {
                            ...
                            if (dat != null && dat.getChildByPath("info") != null) {
                                ...
                                try {
                                    RedisUtil.hset(KEYNAMES.QUEST_COUNT.getKeyName(), String.valueOf(id), mapper.writeValueAsString(info));
            ...
            RedisUtil.hmset(KEYNAMES.NPC_NAME.getKeyName(), npcNames);
        }
    }

    public static boolean exitsQuestCount(int mo, int id) {
        String data = RedisUtil.hget(KEYNAMES.QUEST_COUNT.getKeyName(), String.valueOf(mo));
        if (data == null) {
            return false;
        }
        List<Integer> ret;
    ...
    public static String getNpcName(int nid) {
        return RedisUtil.hget(KEYNAMES.NPC_NAME.getKeyName(), String.valueOf(nid));
    }

    public static int getRandomNPC() {
        int ret = 0;
        Map<String, String> data = RedisUtil.hgetAll(KEYNAMES.NPC_NAME.getKeyName());
        if (data == null) {
            return ret;
        }
