...
public class MobSkillFactory {
    ...
    public static void initialize() {
        if (!RedisUtil.exists(RedisUtil.KEYNAMES.MOBSKILL_DATA.getKeyName())) {
            ...
            for (MapleData skillData : skillRoot.getChildren()) {
                for (MapleData levelData : skillData.getChildByPath("level").getChildren()) {
                    ...
                    try {
                        RedisUtil.hset(RedisUtil.KEYNAMES.MOBSKILL_DATA.getKeyName(), skillId + ":" + level, JsonUtil.getMapperInstance().writeValueAsString(ret));
                    } catch (JsonProcessingException e) {
                        log.fatal("加载怪物技能失败", e);
                    }
                }
            }
            log.info("共加载 " + RedisUtil.hlen(RedisUtil.KEYNAMES.MOBSKILL_DATA.getKeyName()) + " 个怪物技能信息...");
        }
    }
