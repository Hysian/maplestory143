...
public class SkillFactory {
    ...
    public static void loadDelays() {
        if (!RedisUtil.exists(KEYNAMES.DELAYS.getKeyName())) {
            int del = 0; //buster is 67 but its the 57th one!
            for (MapleData delay : delayData) {
                if (!delay.getName().equals("info")) {
                    RedisUtil.hset(KEYNAMES.DELAYS.getKeyName(), delay.getName(), String.valueOf(del));
                    del++;
                }
            }
        }
    ...
    public static void loadSkillData() {
        if (!RedisUtil.exists(KEYNAMES.SKILL_DATA.getKeyName())) {
                        ...
                        if (data.getName().equals("skill")) {
                            for (MapleData data2 : data) { // Loop thru each jobs
                                if (data2 != null) {
                                    ...
                                    job.add(skillid);
                                    try {
                                        RedisUtil.hset(KEYNAMES.SKILL_DATA.getKeyName(), String.valueOf(skillid), mapper.writeValueAsString(skil));
                                        RedisUtil.hset(KEYNAMES.SKILL_NAME.getKeyName(), String.valueOf(skillid), getName(skillid, stringData));
...
//                                        sse.delay = MapleDataTool.getInt("bulletSpeed", summon_data, 0);
                                        ...
                                        try {
                                            RedisUtil.hset(KEYNAMES.SUMMON_SKILL_DATA.getKeyName(), data2.getName(), mapper.writeValueAsString(sse));
                                        } catch (JsonProcessingException e) {
                                            log.error("", e);
                                        }
//                                        SummonSkillInformation.put(skillid, sse);
                                    ...
                                    for (MapleData data3 : data2) {
                                        if (data3.getName().equals("vehicleID")) {
                                            RedisUtil.hset(KEYNAMES.MOUNT_ID.getKeyName(), data2.getName(), MapleDataTool.getString("vehicleID", data2, "0"));
                                        }
                                    }
                                }
                            }
                    ...
                    for (MapleData data : datasource.getData(topDir.getName())) {
                        ...
                        try {
                            RedisUtil.hset(KEYNAMES.FAMILIAR_DATA.getKeyName(), data.getName(), mapper.writeValueAsString(skil));
                        } catch (JsonProcessingException e) {
                            e.printStackTrace();
                        }
                    }
                    ...
                    for (MapleData data : datasource.getData(topDir.getName())) {
                        ...
                        try {
                            RedisUtil.hset(KEYNAMES.CRAFT_DATA.getKeyName(), data.getName(), mapper.writeValueAsString(skil));
                        } catch (JsonProcessingException e) {
                            e.printStackTrace();
                        }
                    }
            ...
            for (Entry<Integer, List<Integer>> entry : skillsByJob.entrySet()) {
                try {
                    RedisUtil.hset(KEYNAMES.SKILL_BY_JOB.getKeyName(), String.valueOf(entry.getKey()), mapper.writeValueAsString(entry.getValue()));
                } catch (JsonProcessingException e) {
                    e.printStackTrace();
                }
            }
            ...
            finalAttackSkills.forEach(integer -> RedisUtil.sadd(KEYNAMES.FINLA_ATTACK_SKILL.getKeyName(), integer.toString()));
        } else {
            Map<String, String> datas = RedisUtil.hgetAll(KEYNAMES.SKILL_DATA.getKeyName());
...
//            forkJoinPool.shutdown();
            ...
            Map<String, String> datas_c = RedisUtil.hgetAll(KEYNAMES.CRAFT_DATA.getKeyName());
            ...
            Set<String> smembers = RedisUtil.smembers(KEYNAMES.FINLA_ATTACK_SKILL.getKeyName());
    ...
    public static void reloadSkills(int skillid) {
        RedisUtil.hdel(KEYNAMES.SKILL_DATA.getKeyName(), String.valueOf(skillid));
    ...
    public static void loadMemorySkills() {
        if (!RedisUtil.exists(KEYNAMES.MEMORYSKILL_DATA.getKeyName())) {
                ...
                try (PreparedStatement ps = con.prepareStatement("SELECT * FROM memoryskills")) {
                    try (ResultSet rs = ps.executeQuery()) {
                        while (rs.next()) {
                             ...
                             */
                            if (RedisUtil.hexists(KEYNAMES.MEMORYSKILL_DATA.getKeyName(), String.valueOf(skillId)) || skill == null || skill.getSkillByJobBook(skillId) == -1) {
                                continue;
                            }
                            RedisUtil.hset(KEYNAMES.MEMORYSKILL_DATA.getKeyName(), String.valueOf(skillId), String.valueOf(skill.getSkillByJobBook(skillId)));
                        }
                    }
                }
    ...
    public static int getIdFromSkillId(int skillId) {
        String ret = RedisUtil.hget(KEYNAMES.MEMORYSKILL_DATA.getKeyName(), String.valueOf(skillId));
        return ret != null ? Integer.valueOf(ret) : 0;
    }

    public static boolean isMemorySkill(int skillId) {
        return RedisUtil.hexists(KEYNAMES.MEMORYSKILL_DATA.getKeyName(), String.valueOf(skillId));
    }

    public static List<Integer> getSkillsByJob(int jobId) {
        List<Integer> ret = null;
        String data = RedisUtil.hget(KEYNAMES.SKILL_BY_JOB.getKeyName(), String.valueOf(jobId));
        if (data != null) {
            try {
    ...
    public static String getSkillName(int id) {
        return RedisUtil.hget(KEYNAMES.SKILL_NAME.getKeyName(), String.valueOf(id));
    }
    ...
    public static Integer getDelay(String id) {
        ...
        String data = RedisUtil.hget(KEYNAMES.DELAYS.getKeyName(), id);
        if (data != null) {
    ...
    public static SummonSkillEntry getSummonData(int skillid) {
        ...
        try {
            String data = RedisUtil.hget(KEYNAMES.SUMMON_SKILL_DATA.getKeyName(), String.valueOf(skillid));
            if (data == null) {
                return null;
            }
    ...
    public static Map<Integer, String> getAllSkills() {
        ...
        Map<String, String> result = RedisUtil.hgetAll(KEYNAMES.SKILL_NAME.getKeyName());
...
//        return null;
    ...
    public static CraftingEntry getCraft(int id) {
        ...
        String data = RedisUtil.hget(KEYNAMES.CRAFT_DATA.getKeyName(), String.valueOf(id));
        if (data != null) {
            try {
    ...
    public static FamiliarEntry getFamiliar(int id) {
        ...
        String data = RedisUtil.hget(KEYNAMES.FAMILIAR_DATA.getKeyName(), String.valueOf(id));
        if (data != null) {
            try {
    ...
    public static int getMountLinkId(int mountid) {
        int ret = 0;
        String data = RedisUtil.hget(KEYNAMES.MOUNT_ID.getKeyName(), String.valueOf(mountid));
        if (data != null) {
